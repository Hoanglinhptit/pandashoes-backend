    ...
    if (deCode === true) {
      ...
      res.json(
        response.success({
          token,
          user: userChek._id,
          name: userChek.firstName + " " + userChek.lastName,
        })
      );
    } else {
...
const register = async (req, res) => {
  const { firstName, lastName, email, passWord, address, initSecret, phone } =
    req.body;
  ...
  res.json(response.success(saveUser));
};
const addUserCms = async (req, res) => {
  ...
  if (
    (keySearch !== "" && firstName.includes(keySearch)) ||
    lastName.includes(keySearch) ||
    role.includes(keySearch)
  ) {
    ...
    User.countDocuments(
      {
        $or: [
          ...
          { role: { $regex: keySearch, $options: "i" } },
        ],
      },
      (err1, data1) => {
